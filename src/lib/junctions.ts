...
export function calculateBothCornerPoints(
  ...
  if (Math.abs(cross) > 0.001) {
    ...
    // Use a single distance for both inner and outer bounds off the node to ensure the cut is 
    // strictly perpendicular to the road. This ensures the sidewalk width across the chamfer is constant.
    const baseT = Math.max(t, ot);
    const baseU = Math.max(u, ou);
    ...
    // To maintain constant sidewalk width, the outer chamfer line must be parallel to the inner chamfer line
    // and separated by the sidewalk width.
    ...
    if (L > 1e-4 && (finalT !== t || finalU !== u)) {
        // Chamfer is active. Calculate the outward normal of the inner chamfer segment.
        ...
        // Point on the target outer chamfer line
        const P = { x: M.x + N.x * sw, y: M.y + N.y * sw };

        // Intersect ray OA + a * dir1 with line P + b * V
        ...
        if (Math.abs(det1) > 1e-5) {
            ...
            finalOT = a; // For outer compatibility if needed, but we just use OA_outer
        }

        // Intersect ray OB + c * dir2 with line P + b * V
