...
export function getExtendedEdgeControlPoints(edge: Edge, nodes: Node[], edges: Edge[], chamferAngleDeg: number): Point[] {
  ...
  const W0 = getEdgeClearance(edge.source, edge, nodes, edges, chamferAngleDeg) + (sourceDegree > 1 ? 14 : 0);
  ...
  if (basePts.length > 2) {
    ...
    const WN = (edge.target ? getEdgeClearance(edge.target, edge, nodes, edges, chamferAngleDeg) : 0) + (targetDegree > 1 ? 14 : 0);
    ...
    res.push({ x: cw0.x + (u1.x - cw0.x)/3, y: cw0.y + (u1.y - cw0.y)/3 });
    res.push({ x: cw0.x + 2*(u1.x - cw0.x)/3, y: cw0.y + 2*(u1.y - cw0.y)/3 });
    res.push(u1);
  }
