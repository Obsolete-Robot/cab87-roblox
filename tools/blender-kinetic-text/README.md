...
Enable `Stroke` in `Text Style` to create a generated mesh outline behind each word. The add-on evaluates a temporary flat version of the word, fills the source face and glyph holes into a solid backing mesh, then adds bounded rounded outer joins so sharp or noisy font points cannot create long miter spikes. The fill Text geometry stays unchanged. The stroke can be styled with color, alpha, width, local Z offset, and independent stroke extrusion.
